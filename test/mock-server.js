...
function useMockServer() {
  const handlers = [
    ...
    http.post('https://www.aemshop.net/cs-graphql', async ({ request }) => {
      const graphqlBody = await request.json();

      if (graphqlBody.variables.sku === '24-MB03') {
